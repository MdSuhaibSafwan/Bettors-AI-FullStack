...
def decompression(json_data) -> Dict[str, Union[str, int, float]]:
    ...
    try:
        return_dict['image_url'] = json_data['image_url']
    except:
        return_dict['image_url'] = None
    return return_dict
...
@sync_to_async
def execute_sync_function(func, *args, **kwargs):
    return func(*args, **kwargs)


def save_file(file):
    path = default_storage.save(f"message/temp/{file.name}", ContentFile(file.read()))
    path_url = f"media/{path}"
    return path_url
