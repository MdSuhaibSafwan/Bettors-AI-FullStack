...
class RequestUtil:
    ...
    # CSRF COOKIE取得
    def get_csrf_cookie(self, request=None):
        if request:
    ...
    # リクエストURL取得
    def get_request_url(self, request=None):
        if request:
            return request.path
        else:
    ...
    # リクエストホストURL（送信元）取得
    def get_request_host_url(self, request=None):
        if request:
    ...
    # タイムゾーン取得
    def get_time_zone(self, request=None):
        if request:
    ...
    # ユーザーエージェント取得
    def get_user_agent(self, request=None):
        if request:
